...
class CWebdebugImageProcessor {
    ...
    var $image_frame_opacity;
		...
		/**
		 * Adds a corners on the image
		 *
     * Value is a local image filename, relative or absolute. PNG is only supported, as well as PNG alpha.
     *
     * Default value is null
     *
     * @access public
     ...
     */
    function CWebdebugImageProcessor($file) {
        ...
        $this->translation        = array();
				$this->translation['file_error']                  = GetMessage('WEBDEBUG_IMAGE_FILE_ERROR');
				$this->translation['local_file_missing']          = GetMessage('WEBDEBUG_IMAGE_LOCAL_FILE_MISSING');
				$this->translation['local_file_not_readable']     = GetMessage('WEBDEBUG_IMAGE_LOCAL_FILE_NOT_READABLE');
				$this->translation['uploaded_too_big_ini']        = GetMessage('WEBDEBUG_IMAGE_UPLOADED_TOO_BIG_INI');
				$this->translation['uploaded_too_big_html']       = GetMessage('WEBDEBUG_IMAGE_UPLOADED_TOO_BIG_HTML');
				$this->translation['uploaded_partial']            = GetMessage('WEBDEBUG_IMAGE_UPLOADED_PARTIAL');
				$this->translation['uploaded_missing']            = GetMessage('WEBDEBUG_IMAGE_UPLOADED_MISSING');
				$this->translation['uploaded_unknown']            = GetMessage('WEBDEBUG_IMAGE_UPLOADED_UNKNOWN');
				$this->translation['try_again']                   = GetMessage('WEBDEBUG_IMAGE_TRY_AGAIN');
				$this->translation['file_too_big']                = GetMessage('WEBDEBUG_IMAGE_FILE_TOO_BIG');
				$this->translation['no_mime']                     = GetMessage('WEBDEBUG_IMAGE_NO_MIME');
				$this->translation['incorrect_file']              = GetMessage('WEBDEBUG_IMAGE_INCORRECT_FILE');
				$this->translation['image_too_wide']              = GetMessage('WEBDEBUG_IMAGE_IMAGE_TOO_WIDE');
				$this->translation['image_too_narrow']            = GetMessage('WEBDEBUG_IMAGE_IMAGE_TOO_NARROW');
				$this->translation['image_too_high']              = GetMessage('WEBDEBUG_IMAGE_IMAGE_TOO_HIGH');
				$this->translation['image_too_short']             = GetMessage('WEBDEBUG_IMAGE_IMAGE_TOO_SHORT');
				$this->translation['ratio_too_high']              = GetMessage('WEBDEBUG_IMAGE_RATIO_TOO_HIGH');
				$this->translation['ratio_too_low']               = GetMessage('WEBDEBUG_IMAGE_RATIO_TOO_LOW');
				$this->translation['too_many_pixels']             = GetMessage('WEBDEBUG_IMAGE_TOO_MANY_PIXELS');
				$this->translation['not_enough_pixels']           = GetMessage('WEBDEBUG_IMAGE_NOT_ENOUGH_PIXELS');
				$this->translation['file_not_uploaded']           = GetMessage('WEBDEBUG_IMAGE_FILE_NOT_UPLOADED');
				$this->translation['already_exists']              = GetMessage('WEBDEBUG_IMAGE_ALREADY_EXISTS');
				$this->translation['temp_file_missing']           = GetMessage('WEBDEBUG_IMAGE_TEMP_FILE_MISSING');
				$this->translation['source_missing']              = GetMessage('WEBDEBUG_IMAGE_SOURCE_MISSING');
				$this->translation['destination_dir']             = GetMessage('WEBDEBUG_IMAGE_DESTINATION_DIR');
				$this->translation['destination_dir_missing']     = GetMessage('WEBDEBUG_IMAGE_DESTINATION_DIR_MISSING');
				$this->translation['destination_path_not_dir']    = GetMessage('WEBDEBUG_IMAGE_DESTINATION_PATH_NOT_DIR');
				$this->translation['destination_dir_write']       = GetMessage('WEBDEBUG_IMAGE_DESTINATION_DIR_WRITE');
				$this->translation['destination_path_write']      = GetMessage('WEBDEBUG_IMAGE_DESTINATION_PATH_WRITE');
				$this->translation['temp_file']                   = GetMessage('WEBDEBUG_IMAGE_TEMP_FILE');
				$this->translation['source_not_readable']         = GetMessage('WEBDEBUG_IMAGE_SOURCE_NOT_READABLE');
				$this->translation['no_create_support']           = GetMessage('WEBDEBUG_IMAGE_NO_CREATE_SUPPORT');
				$this->translation['create_error']                = GetMessage('WEBDEBUG_IMAGE_CREATE_ERROR');
				$this->translation['source_invalid']              = GetMessage('WEBDEBUG_IMAGE_SOURCE_INVALID');
				$this->translation['gd_missing']                  = GetMessage('WEBDEBUG_IMAGE_GD_MISSING');
				$this->translation['watermark_no_create_support'] = GetMessage('WEBDEBUG_IMAGE_WATERMARK_NO_CREATE_SUPPORT');
				$this->translation['watermark_create_error']      = GetMessage('WEBDEBUG_IMAGE_WATERMARK_CREATE_ERROR');
				$this->translation['watermark_invalid']           = GetMessage('WEBDEBUG_IMAGE_WATERMARK_INVALID');
				$this->translation['file_create']                 = GetMessage('WEBDEBUG_IMAGE_FILE_CREATE');
				$this->translation['no_conversion_type']          = GetMessage('WEBDEBUG_IMAGE_NO_CONVERSION_TYPE');
				$this->translation['copy_failed']                 = GetMessage('WEBDEBUG_IMAGE_COPY_FAILED');
				$this->translation['reading_failed']              = GetMessage('WEBDEBUG_IMAGE_READING_FAILED');

        // determines the language
				$lang = GetMessage("WEBDEBUG_IMAGE_LANG");
        $this->lang               = $lang;
				/*
        ...
        if ($this->processed) {
            ...
            if ($image_manipulation) {
                ...
                if ($this->processed && $image_src) {
                    ...
                    // add corners
                    if ($this->image_corners!='' && file_exists($this->image_corners)) {
                        $this->log .= '- add corners<br />';
                        $corners_info = getimagesize($this->image_corners);
												$corners_type = (array_key_exists(2, $corners_info) ? $corners_info[2] : null); // 1 = GIF, 2 = JPG, 3 = PNG
                        $corners_checked = false;
                        if ($corners_type == IMAGETYPE_GIF) {
                            if (!function_exists('imagecreatefromgif')) {
                                $this->error = $this->translate('corners_no_create_support', array('GIF'));
                            } else {
                                $filter = @imagecreatefromgif($this->image_corners);
                                if (!$filter) {
                                    $this->error = $this->translate('corners_create_error', array('GIF'));
                                } else {
                                    $this->log .= '&nbsp;&nbsp;&nbsp;&nbsp;corners source image is GIF<br />';
                                    $corners_checked = true;
                                }
                            }
                        } else if ($corners_type == IMAGETYPE_JPEG) {
                            if (!function_exists('imagecreatefromjpeg')) {
                                $this->error = $this->translate('corners_no_create_support', array('JPEG'));
                            } else {
                                $filter = @imagecreatefromjpeg($this->image_corners);
                                if (!$filter) {
                                    $this->error = $this->translate('corners_create_error', array('JPEG'));
                                } else {
                                    $this->log .= '&nbsp;&nbsp;&nbsp;&nbsp;corners source image is JPEG<br />';
                                    $corners_checked = true;
                                }
                            }
                        } else if ($corners_type == IMAGETYPE_PNG) {
                            if (!function_exists('imagecreatefrompng')) {
                                $this->error = $this->translate('corners_no_create_support', array('PNG'));
                            } else {
                                $filter = @imagecreatefrompng($this->image_corners);
                                if (!$filter) {
                                    $this->error = $this->translate('corners_create_error', array('PNG'));
                                } else {
                                    $this->log .= '&nbsp;&nbsp;&nbsp;&nbsp;corners source image is PNG<br />';
                                    $corners_checked = true;
                                }
                            }
                        } else if ($corners_type == IMAGETYPE_BMP) {
                            if (!method_exists($this, 'imagecreatefrombmp')) {
                                $this->error = $this->translate('corners_no_create_support', array('BMP'));
                            } else {
                                $filter = @$this->imagecreatefrombmp($this->image_corners);
                                if (!$filter) {
                                    $this->error = $this->translate('corners_create_error', array('BMP'));
                                } else {
                                    $this->log .= '&nbsp;&nbsp;&nbsp;&nbsp;corners source image is BMP<br />';
                                    $corners_checked = true;
                                }
                            }
                        } else {
                            $this->error = $this->translate('watermark_invalid');
												...
												list($CornerW, $CornerH) = getimagesize($this->image_corners);
												...
												{
													$border_color = imagecolorat($filter, $i, $cropsize);
													$arBorderColor[] = $border_color;
													if($border_color && !in_array($border_color, $arBorderColor))
													{
														$arBorderColor[] = $border_color;
													}
												}
												...
												foreach ($arBorderColor as $key=>$border_color)
												{
													//imageRectangle($image_dst, 0+$key, 0+$key, $width_destination-1-$key, $height_destination-1-$key, $border_color);
												}
												
												
												for ($i=1; $i<=4; $i++)
												{
													$cor = imagecreatetruecolor($cropsize, $cropsize);
													imagealphablending($cor, false);
													...
													imagecopyresampled($cor, $filter, 0, 0, $cropx, $cropy, $cropsize, $cropsize, $cropsize, $cropsize);
													...
													switch ($i)
													{
														case 1:
															break;
														case 2:
															$copyx = $this->image_dst_x-$cropsize;
															break;
														case 3:
															$copyy = $this->image_dst_y-$cropsize;
															break;
														case 4:
															$copyx = $this->image_dst_x-$cropsize;
															$copyy = $this->image_dst_y-$cropsize;
													...
													imagecopy($image_dst, $cor, $copyx, $copyy, 0, 0, $cropsize, $cropsize);
												}
                    }
                    ...
                    switch($this->image_convert) {
                        ...
                        case 'png':
                            ...
                            if (!$return_mode) {
                                ...
                                ob_start();
                                $result = @imagepng($image_dst, NULL);
                                $return_content = ob_get_contents();
                                ob_end_clean();
                            }
                            if (!$result) {
                        ...
                        case 'gif':
                            if (!$return_mode) {
                                ...
                                ob_start();
                                $result = @imagegif($image_dst, NULL);
                                $return_content = ob_get_contents();
                                ob_end_clean();
                            }
                            if (!$result) {
                        ...
                        case 'bmp':
                            if (!$return_mode) {
                                ...
                                ob_start();
                                $result = $this->imagebmp($image_dst, NULL);
                                $return_content = ob_get_contents();
                                ob_end_clean();
                            }
                            if (!$result) {
