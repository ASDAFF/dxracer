...
; /* Start:"a:4:{s:4:"full";s:91:"/local/templates/main/components/bitrix/sale.basket.basket/basket/script.js?160918921037921";s:6:"source";s:75:"/local/templates/main/components/bitrix/sale.basket.basket/basket/script.js";s:3:"min";s:0:"";s:3:"map";s:0:"";}"*/
...
BasketPoolQuantity.prototype.updateQuantity = function () {
	...
	if (!!items && items.children.length > 0) {
		for (var i = 0; items.children.length > i; i++) {
			var itemId = items.children[i].id;
...
function updateBasketTable(basketItemId, res) {
	...
	lastRow = children[children.length - 1];
	...
	if (!!res.BASKET_DATA) {
		...
		if (BX('allSum_FORMATED')){
			...
			BX('sum_FORMATED').innerHTML = res['BASKET_DATA']['allSum_FORMATED'].replace(/\s/g, '&nbsp;');
		}
...
function recalcBasketAjax(params) {
	...
	if (!!delayedItems && delayedItems.children.length > 0) {
		for (i = 0; delayedItems.children.length > i; i++)
...
function checkAnalytics(currentQuantity, newItems) {
	...
	if (!!newItems && newItems.children.length) {
		for (i = 0; newItems.children.length > i; i++) {
			itemId = newItems.children[i].id;
			diff = BX('QUANTITY_' + itemId).value - currentQuantity[itemId];

			if (diff != 0) {
				current = getCurrentItemAnalyticsInfo(newItems.children[i], diff);
...
/* End */
;; /* /local/templates/main/components/bitrix/sale.basket.basket/basket/script.js?160918921037921*/
