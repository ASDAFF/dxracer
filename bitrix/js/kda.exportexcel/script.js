...
var EList = {
	...
	ShowListSettings: function(btn)
	{
		...
		var dialog = new BX.CAdminDialog({
			'title':'',
			'content_url':'/bitrix/admin/kda_import_excel_list_settings.php',
			'content_post': post,
			'width':'900',
			'height':'400',
			'resizable':true});
...
var EImport = {
	...
	GetStatus: function()
	{
		...
		$.ajax({
			type: "GET",
			url: '/upload/tmp/'+kdaIEModuleName+'/'+this.pid+'.txt?hash='+(new Date()).getTime(),
			success: function(data){
				var finish = false;
...
$(document).ready(function(){
	...
	if($('#kda-ee-updates-message').length > 0)
	{
		$.post('/bitrix/admin/'+kdaIEModuleFilePrefix+'.php?lang='+BX.message('LANGUAGE_ID'), 'MODE=AJAX&ACTION=SHOW_MODULE_MESSAGE', function(data){
			data = $(data);
			var inner = $('#kda-ee-updates-message-inner', data);
			if(inner.length > 0 && inner.html().length > 0)
			{
				$('#kda-ee-updates-message-inner').replaceWith(inner);
				$('#kda-ee-updates-message').show();
			}
		});
	}
});