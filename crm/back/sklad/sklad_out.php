...
(
SELECT `model` FROM `sklad_invoice_data` WHERE `date_get` IS NULL AND `not_obtained`=0
UNION ALL
   (
   SELECT SC.model as model FROM `sklad_coming` AS SC
  WHERE SC.id NOT IN (
  SELECT model_id
  FROM `sklad_sales`
  UNION
...
if (isset($_REQUEST['exel'])) {
    ...
    header('Content-Disposition: attachment; filename=Склад_едет' . $data . '.xls');
