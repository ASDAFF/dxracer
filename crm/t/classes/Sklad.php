...
class Sklad
{
  ...
  function action($action, $data = "", $model_id = "", $post = "")
  {
    ...
    switch ($action) {
      ...
      case 'addPos2Invoce':
        ...
        return $invoice->addPos2Invoce($post);
        break;
  ...
  private function setComing()
  {
    ...
    foreach ($arr_data as $value_str) {
      ...
      $arr_out['count'] = 1;
      if (is_numeric($arr_out[1])) {
        $arr_out['count'] = $arr_out[1];
      }
      ...
      if(preg_replace("/[^A-Za-z]/", '', $model)==='T'){
        $sort = 10;
        $brand = "Tissot";
      }elseif (preg_match('/\w+\.\w+/', $model)){
        $sort = 50;
        $brand = 'Calvin Klein';
      }else{
        $sort = 100;
      ...
      if ($arr_out['count'] > 1) {
        for ($r = 0; $r != $arr_out['count']; $arr_out['count']--) {
  ...
  private function setSales()
  {
    ...
    foreach ($arr_data as $value) {
      $i = 0;
      if (preg_match('/MS/', $value[$i])) {
        $project = "MS";
      } elseif (preg_match('/KW/', $value[$i])) {
        $project = "KW";
      } elseif (preg_match('/TA/', $value[$i])) {
        $project = "TA";
      }
      elseif (preg_match('/MX/', $value[$i])) {
        $project = "MX";
      }
...
//            $comment = str_replace("%", '\%', $comment);
    ...
    /**добавляем в таблицу для логирования**/
    $logs = new Logs();
    if (!$logs->setLog($sql_ins, $table_name)) {
      return json_encode(array("result" => 0, "text" => " {$logs->error_text}"));
    }
...
//                }
  ...
  {
    file_get_contents("https://time-avenue.com/api/?paid=1&auth=XDFDD3e45fvsdfvvfg4&email={$email}");
  }
...
//            $comment = trim($value[$i + 6]);
      ...
      preg_match('#RU\d{9,10}#', $value[$i + 6], $value_dpd_2);

      $value_dpd_2 = $value_dpd_2[0];
      if (!empty($value_dpd_2)) {
        $shipnumber = $value_dpd_2;
        $shipper = 'DPD';
      }
      ...
      preg_match('#GURU\d{3,15}#', $value[$i + 6], $value_guru);

      $value_guru = $value_guru[0];
      if (!empty($value_guru)) {
        $shipnumber = $value_guru;
        $shipper = 'GURU';
      }

      preg_match('#ГУРУ\d{3,15}#', $value[$i + 6], $value_guru);
...
//            }
  ...
  private function setComingMoney()
  {
    ...
    foreach ($arr_data as $value) {
      ...
      if (count($elements_model_id) > 0) { //модели которые уже использовали
        $elements = implode(",", $elements_model_id);
        $dop_where = "AND model_id NOT IN(" . $elements . ")";
      } else {
        $dop_where = "";
      }

      $sql = "SELECT model_id FROM `sklad_shipment` WHERE (phone LIKE '%{$phone1}%' {$sql_phone_2}) AND `model`='{$model}' AND `date_1`='{$date_1}'
                    AND model_id NOT IN(SELECT model_id FROM `sklad_coming_money`) {$dop_where}
    ...
    if ($error) {
      return json_encode(array("result" => 0, "text" => $sql_ins . "<br>" . $value_sql . "<br>" . $sql . "<br>" . $last_query));
    }
...
//                }
  ...
  private function setDiscard()
  {
    ...
    if ($this->DB->insert($sql)) {
      ...
      $model_name = $this->getModelByID($this->model_id);
      $sql = "INSERT INTO {$table_name} (`id`,`date_insert`,`model`,`model_id`, `comment`) VALUES (NULL,'{$this->timestamp}','$model_name','{$this->model_id}','{$this->data}')";

      $q[] = $sql;
      if (!$logs->setLog($q, $table_name)) {
        return json_encode(array("result" => 0, "text" => " {$logs->error_text}"));
      }
  ...
  private function setReturn()
  {
    ...
    if (!$this->getReturn()) {
      ...
      if ($this->DB->insert($sql)) {
        ...
        $logs = new Logs();
        $model_name = $this->getModelByID($this->model_id);
        $sql_ins[] = "INSERT INTO {$table_name} (`id`,`date_insert`,`model`,`model_id`, `comment`,`phone`) VALUES (NULL, '{$this->timestamp}','{$model_name}','{$this->model_id}','{$this->data}','{$phone}')";
...
//        <th>Дата ухода</th>
   ...
   * @param array $brand
   */
  public function syncBrand($brand = array('Diesel'))
  {


    $for_delete = array();
    $table_name = "`sklad_coming`"; //удаленный сервак
    $settings = array(
            'server' => '188.225.80.5',
            'username' => 'crm.kw',
            'password' => 'SoZkqm6AzvTHNfrL',
            'db' => 'zakaz1234',
            'port' => 3306,
            'charset' => 'utf8',
    );
    ...
    //получаем позиции, которые необходимо удалить из текущего склада
    $sql = "SELECT model,id FROM `sklad_coming`
                WHERE
 				(id IN(SELECT model_id FROM sklad_sales) OR 
                id IN(SELECT model_id FROM sklad_shipment) OR 
                id IN(SELECT model_id FROM sklad_discard)) AND
                brand = '{$brand[0]}'";

    $res = $DB->simpleQuery($sql); //получили только те позиции, что в наличии
    while ($arr = $res->fetch_assoc()) {
      $for_delete[] = $arr['id'];

      $sql = "DELETE FROM {$table_name} WHERE `id_sync`={$arr['id']}";
      $this->DB->Delete($sql);

    }


    //получаем позиции которые необходимо добавить
    ...
    while ($arr = $res->fetch_assoc()) {
      ...
      if (in_array($arr['id'], $for_delete)) { //пропускаем если позиция продана и еще, что то с ней
        continue;
      }

//добавляем по одной позиции те, что если не добавятся
      $sql = "
                INSERT INTO {$table_name} (
                `id`,
                `id_sync`,
                ...
                VALUES (NULL,'{$arr['id']}','{$timestamp}','{$arr['model']}','W','110','{$brand[0]}')";
      $this->DB->simpleQuery($sql);
...

  {
    ...
    system('/usr/bin/mysqldump -u'.Configuration::MYSQL_USERNAME.' -p'.Configuration::MYSQL_PASSWORD.' --opt --complete-insert '.Configuration::MYSQL_DB.' |  gzip > 
    /home/bitrix/www/crm/t/sklad/close_sklad_sql/zakaz1234_`date +\%d-\%m-\%Y`.sql.gz');
...
//--------------------------------------------
    ...
    $save_statistics = base64_encode(file_get_contents('https://sklad:132449@crm.time-avenue.com/t/sklad/statistic.php', false, stream_context_create($stream_opts))); //сохраняем статистику
...
//            $arr_out[$res[0]] = $res;
    ...
    while ($res_arr = $arr->fetch_row()) {
      ...
      foreach ($arr1 as $key => $value) {
        $arr_out[$key]["cnt"] += $value["cnt"];
        $arr_out[$key]["cnt_skld"] = 0;
      }
    }

    $cur_sklad = $this->getCurrentSklad();
//
    foreach ($cur_sklad as $value) {
      if (key_exists($value["model"], $arr_out)) {
        $arr_out[$value["model"]]["cnt_skld"] += 1;
      }
    }
   ...
   */
  private function getCurrentSklad()
  {
    $sql = "SELECT
                      SC.model,
                      count(model) AS cnt
...
//    }
  ...
  private function setSkladFullExportLog()
  {
    ...
    $this->data = base64_encode($this->data);
    $date_to = date("d.m.Y H:i:00");
//        $date_to = "31.07.2017 20:13:58";
    $date_from = $this->getDateLastReOrder();
    $save_reorder_json = json_encode($this->reOrder(false, $date_from, $date_to));

    $save_statistics = base64_encode(file_get_contents('http://sklad:132449@crm.time-avenue.com/t/sklad/statistic.php')); //сохраняем статистику
    $save_ExecutedOrders = base64_encode($this->getExecutedOrders()); //сохраняем выполненые заказы
    $save_refusals = base64_encode($this->getRefusals()); //сохраняем отказы
    $save_problems = base64_encode($this->getProblems()); //сохраняем проблеммы
    $save_other = base64_encode($this->getOther()); //сохраняем разное
    ...
    $sql = "INSERT INTO `sklad_full_export_log` (`id`, `date_insert`, `base64`,`save_reorder_json`,`save_statistics`,`sales`,`refusals`,`problems`,`other`)
                 VALUES (NULL, '{$this->timestamp}', '{$this->data}','{$save_reorder_json}','{$save_statistics}','{$save_ExecutedOrders}','{$save_refusals}','{$save_problems}','{$save_other}')";
  ...
  function getOneReorder($id)
  {
    ...
    $full_arr = json_decode($arr, true);
    foreach ($full_arr as $key => $value) {

      $return_arr[$key]["cnt"] = $value['cnt'];
      $return_arr[$key]["cnt_skld"] = $value['cnt_skld'];
    }
...
//
//        foreach ($cur_sklad as $value) {
//            if(key_exists($value["model"], $return_arr)) {
//                $return_arr[$value["model"]]["cnt_skld"] += 1;
//            }
//        }
  ...
  {
    $query = "SELECT model FROM `sklad_coming` WHERE id = {$id}";
