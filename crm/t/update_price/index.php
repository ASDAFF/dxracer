    ...
    <form id="all">
        <table id="generalTable" class="table table-striped table-bordered table-hover" cellspacing="0" width="100%">
            ...
            <? for ($i = 0; $i < count($arr); $i++) {
                ...
                if ($arr[$i]["specialofferms"]) {
                    $specms = 1;
                    $specms_checked = "checked";
                } else {
                    $specms = 0;
                    $specms_checked = "";
                }
...
//                    $dnt_chn_price= 0;
                ...
                echo "
                    <tr id='{$arr[$i]["model"]}' style='background-color:{$arr[$i]["color"]} '>
                      ...
                      <td style='background: #3c763d;'>
                        <div class=\"checkbox\">
                          <label><input id=\"dnt_chn_price-{$arr[$i]["model"]}\" type=\"checkbox\" value=\"{$dnt_chn_price}\" {$dnt_chn_price_checked}></label>
                        </div>
                        </td>
                        ...
                        <td>
                        <div class=\"checkbox\">
                          <label><input id=\"specms-'{$arr[$i]["model"]}'\" type=\"checkbox\" value=\"{$specms}\" {$specms_checked}></label>
                        </div>
                        </td>
                        <td>
    ...
    <div class="row">
        <div class="col-lg-12 fixed">
	        ...
	        <button type="button" class="btn btn-warning" data-toggle="modal" data-target="#dontChangePriceList">Не менять цену</button>
            <button id="startHandUpdate" type="button" class="btn btn-danger">Запуск парсинга</button>
	        <a type="button" class="btn btn-warning" href="/t/update_price/export2exel.php">Выгрузка в EXEL</a>
            <!--            <span>--><? //= $all_count ?><!--</span>-->
        </div>
...
<div class="modal fade" id="dontChangePriceList" role="dialog">
	<div class="modal-dialog modal-sm">
		<form name="dontChangePriceList_form">
			<div class="modal-content">
				<div class="modal-body">

					<textarea class="form-control" name="list_positions" style="height: 400px"></textarea>

				</div>
				<div class="modal-footer" style="text-align: center;">
					<button type="submit" class="btn btn-info">Внести</button>
				</div>
			</div>
		</form>
	</div>
