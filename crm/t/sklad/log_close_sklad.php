...
//    $base64_out_arr = base64_decode($base64_out_arr);
    ...
    if ($_GET["type"] == 'SC')
        $file_name = "Склад KW {$month_year}";
    elseif ($_GET["type"] == 'SFEL')
        $file_name = "Предварительная выгрузка";
    else
        $file_name = "Статистика KW {$month_year}";
...
<ul>
    <? while ($value = $arr->fetch_assoc()) { ?>
		...
		<li>
			<a href="?id=<?= $value["id"] ?>&type=<?= $value["type"] ?>"> <?= $value["type"] == "SC" ? "Склад KW $month_year" : "Предварительная выгрузка" ?> (<?= $value["date_insert"] ?>)</a>
            <? if ($value["type"] == 'SC') { ?>
				<br>
				<a href="?id=<?= $value["id"] ?>">Статистика KW <?= $month_year ?> (<?= $value["date_insert"] ?>)</a>
            <? } ?>
		</li>
    <? } ?>
</ul>
