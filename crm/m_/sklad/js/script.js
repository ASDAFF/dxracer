$(function () {
    ...
    $('#re_ordering').on('show.bs.modal', function (e) {
        $("#date_to").val(moment().format('D.MM.YYYY HH:mm'));
        $.ajax({
            type: "post",
            url: "actions.php",
            ...
            success: function (text) {
                $("#date_from").val(text);
                $('#date_to').data("DateTimePicker").minDate(text);
            }
        })
    });

    $('#date_from').datetimepicker({locale: 'ru'});
