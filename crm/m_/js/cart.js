$(document).ready(function () {
    ...
    $("#citys").on('focusout', function () {
        ...
        $.ajax({
            ...
            cache: false,
            async: false,
            url: "//crm.monarchshop.ru/m/ajax/get_city.php?term=" + encodeURIComponent(city) + "&one=1",
    ...
    $("#citys").autocomplete({
            source: "//crm.monarchshop.ru/m/ajax/get_city.php",
            minLength: 3,
            autoFocus:true,
            cacheLength: 10,
            selectFirst: true,
...
function getCitiesCashPay(){
    ...
    $.ajax({
            type: "POST",
            url: "//crm.monarchshop.ru/m/ajax/get_city.php?only_spsr=1",
            data: {AOGUID: AOGUID, submit: 'ok', REGIONCODE: REGIONCODE}
        })
        .done(function (msg) {
            if(msg.length==0)
                msg='Только почта';
...
function getbyart() {
    ...
    var url = 'https://monarchshop.ru/api/Utils.php';

    var art = $("#art").val();
    $.get(url, {auth: auth, art: art, tip: 'get'}, function (data) {
        $('#elements').html(data);
        $('.fancybox').fancybox();
