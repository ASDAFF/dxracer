...
//echo "http://cp.marketparser.ru/api/v2/campaigns/{$this->campaigns_id}/reports/{$this->report_id}/results.json{$dop_url}"."<br>";
    ...
    {
        $result_json = UTILS::getByCurl("https://monarchshop.ru/api/?getSpecialOffers=1&auth=XDFDD3e45fvsdfvvfg4", "", false);
...
//    }
    ...
    {
        $query = "SELECT PA.model,CNT_M.cnt, cur_price,PA.recalc_price, competitor, price,specialoffer,color,percent_change,dnt_chn_price
...
//        }
        ...
        /**
         * 1. У товаров сделать свойство СПЕЦПРЕДЛОЖЕНИЕ, отображать его здесь http://crm.monarchshop.ru/m/update_price/ (9PM пока не надо)
        ...
        >= 40000
         */
        $query = "SELECT model,cur_price FROM  `price_all` WHERE specialoffer=1";
        $arr = $this->DB->select($query);
        foreach ($arr as $value) {
            if($value['cur_price']<12000)
                $value["cur_price"] = ceil(($value["cur_price"] - ($value["cur_price"]*$arr_params['сoef2Spec12000']/100))/100)*100;

            if($value['cur_price']<15000 AND $value['cur_price']>=12000)
                $value["cur_price"] = ceil(($value["cur_price"] - ($value["cur_price"]*$arr_params['сoef2Spec1215']/100))/100)*100;

            if($value['cur_price']<20000 AND $value['cur_price']>=15000)
                $value["cur_price"] = ceil(($value["cur_price"] - ($value["cur_price"]*$arr_params['сoef2Spec1520']/100))/100)*100;

            if($value['cur_price']<27000 AND $value['cur_price']>=20000)
                $value["cur_price"] = ceil(($value["cur_price"] - ($value["cur_price"]*$arr_params['сoef2Spec2027']/100))/100)*100;

            if($value['cur_price']<40000 AND $value['cur_price']>=27000)
                $value["cur_price"] = ceil(($value["cur_price"] - ($value["cur_price"]*$arr_params['сoef2Spec2740']/100))/100)*100;

            if($value['cur_price']>=40000)
                $value["cur_price"] = ceil(($value["cur_price"] - ($value["cur_price"]*$arr_params['сoef2Spec40000']/100))/100)*100;


            $query = "UPDATE price_all SET recalc_price = {$value["cur_price"]} WHERE model LIKE '{$value["model"]}'";
            $this->DB->update($query);
        }
/**
 *
