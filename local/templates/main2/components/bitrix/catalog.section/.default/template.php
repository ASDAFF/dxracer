...
foreach ($arResult['ITEMS'] as $key => $arItem)
{
	...
	<div class='label-stic'>
		<?if(!empty($arItem['DISPLAY_PROPERTIES']['HIT'])){?>
...
<?endif;?>
<?if(!empty($arItem['DISPLAY_PROPERTIES']['SALE']) ):?>
...
<?endif;?>
	...
	<div class="bx_catalog_item_price"><div id="<? echo $arItemIDs['PRICE']; ?>" class="bx_price"><?if($arItem['PRICES']['OLD_PRICE']['VALUE']!=0 && !empty($arItem['PRICES']['OLD_PRICE']['VALUE'])):?>
	<?
	if (!empty($minPrice))
	{
		...
		{
			//echo "<pre>"; print_r($arItem); echo "</pre>";
			echo $minPrice['DISCOUNT_VALUE'];?><span class='cur'>РУБ</span><span class='oldprice'><?=$arItem["PRICES"]['OLD_PRICE']['VALUE']?></span>
			<?
		}
		if ('Y' == $arParams['SHOW_OLD_PRICE'] && $minPrice['DISCOUNT_VALUE'] < $minPrice['VALUE'])
		{
			?> <span><? echo $minPrice['VALUE']; ?></span><?
		}
	}
	unset($minPrice);
	?><?endif;?></div></div>
	...
	if (!isset($arItem['OFFERS']) || empty($arItem['OFFERS']))
	{
		...
		if ($arItem['PROPERTIES']['PROP13']['VALUE_ENUM_ID']==38)
		{
			
					?>
		<div id="<? echo $arItemIDs['BASKET_ACTIONS']; ?>" class="bx_catalog_item_controls_blocktwo">
			<a id="<? echo $arItemIDs['BUY_LINK']; ?>" class="btn-ch js-add-<?=$arItem['ID']?> " href="javascript:void(0)" rel="nofollow">В корзину<span><img src="/local/templates/main/img/arBasket.png"></span></a>
			<?// echo "<pre>"; print_r($arItem); echo "</pre>";
			...
			<script>
				$(".js-add-<?=$arItem['ID']?>").click(
					function(){
						...
						"price": <?=$arItem["PRICES"]['BASE']['VALUE']?>, //стоимость единицы товара
						"brand": "<?=$BRAND?>",
		...
		else
		{
			...
			?><div id="<? echo $arItemIDs['NOT_AVAILABLE_MESS']; ?>" class="bx_catalog_item_controls_blockone"><a class="btn-ch js-see-<?=$arItem['ID']?>" href='<? echo $arItem['DETAIL_PAGE_URL']; ?>'>Подробнее<span><img src="/local/templates/main/img/arBasket.png"></span></a>
<script>
				$(".js-see-<?=$arItem['ID']?>").click(
					function(){
						dataLayer.push({
						"ecommerce": {
						"detail": {
