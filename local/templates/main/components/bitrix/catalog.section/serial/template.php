...
foreach ($arResult['ITEMS'] as $key => $arItem)
{
	...
	?><div class="<? echo ($arItem['SECOND_PICT'] ? 'bx_catalog_item double' : 'bx_catalog_item'); ?> col-xs-12 col-lg-3"><div class="bx_catalog_item_container col-xs-12" id="<? echo $strMainID; ?>">
		<?if(!empty($arItem['PROPERTIES']['HIT']) || !empty($arItem['DISPLAY_PROPERTIES']['SALE']) ):?>
	<div class='label-stic'>
		...
		<?}?>
		<div></div>
		<?if(!empty($arItem['PROPERTIES']['HIT'])){?>
...
<?endif;?>
	...
	<div class="bx_catalog_item_price col-xs-6"><div id="<? echo $arItemIDs['PRICE']; ?>" class="bx_price"><?
	if (!empty($minPrice))
	{
		...
		{
		?><span class='oldprice'><?=$arItem["PRICES"]['OLD_PRICE']['VALUE']?></span>
			<?echo $minPrice['DISCOUNT_VALUE'];?><span class='cur'>РУБ</span>
			<?
		}
	...
	{
		?>
		<?// echo "<pre>"; print_r($arItem['PROPERTIES']['PROP13']); echo "</pre>";?>
		<div class="bx_catalog_item_controls col-xs-6"><?
		...
		else
		{
			...
			?><div id="<? echo $arItemIDs['NOT_AVAILABLE_MESS']; ?>" class="bx_catalog_item_controls_blockone"><a class="btn-ch js-see-<?=$arItem['ID']?>" href='<? echo $arItem['DETAIL_PAGE_URL']; ?>'>Подробнее</a>
<script>
